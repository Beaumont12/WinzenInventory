    ...
    const fetchData = async () => {
      ...
      if (salesSnapshot.exists()) {
        ...
        salesSnapshot.forEach((childSnapshot) => {
          total += childSnapshot.val().total; // Assuming total sales is stored under 'total' key
          const customerName = childSnapshot.val().customerName; // Assuming customer name is stored under 'customerName' key
        ...
        setCancelledCount(cancelledSnapshot.exists() ? Object.keys(cancelledSnapshot.val()).length : 0);

        // Fetch count of completed orders
        setCompletedCount(salesSnapshot.exists() ? Object.keys(salesSnapshot.val()).length : 0);
        ...
        <div className='border border-gray-300 p-4 rounded-lg bg-gray-100 flex items-center justify-between'>
          ...
          <div className="flex flex-row items-center space-x-4 bg-blue-300 py-4 px-6 rounded-lg shadow-lg">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-md text-black"><strong>Email:</strong> {adminEmail}</p>
              <p className="text-md text-black"><strong>Username:</strong> {adminUsername}</p>
            </div>
          </div>
        </div>
